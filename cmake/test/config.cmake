...
if (NOT TARGET panmap_test)
    add_library (panmap_test INTERFACE)
    target_link_libraries (panmap_test INTERFACE GTest::gtest_main panmap_lib)
    add_library (panmap::test ALIAS panmap_test)
endif ()

# Add the check target that builds and runs tests.
add_custom_target (check COMMAND ${CMAKE_CTEST_COMMAND} ${CMAKE_CTEST_ARGUMENTS})

get_directory_property (panmap_targets DIRECTORY "${panmap_SOURCE_DIR}/src" BUILDSYSTEM_TARGETS)
foreach (target IN LISTS panmap_targets)
    get_target_property (type ${target} TYPE)
    if (type STREQUAL "EXECUTABLE")
        list (APPEND panmap_EXECUTABLE_LIST ${target})
    endif ()
endforeach ()
unset (panmap_targets)

macro (add_app_test test_filename)
    file (RELATIVE_PATH source_file "${panmap_SOURCE_DIR}" "${CMAKE_CURRENT_LIST_DIR}/${test_filename}")
    get_filename_component (target "${source_file}" NAME_WE)

    add_executable (${target} ${test_filename})
    target_link_libraries (${target} panmap::test)

    add_dependencies (${target} ${panmap_EXECUTABLE_LIST})
