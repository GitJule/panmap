                            ...
                            unsigned seed = 42) 
{
    ...
    seqan3::sequence_file_input fasta_out{};
    
    auto search_tree = libjst::make_volatile(jst_data)
        | libjst::labelled()
        | libjst::trim(pattern_length - 1)
        | libjst::merge(libjst::adjacent)
        | libjst::seek();
    ...
    for (auto const& node : traverser) { 
        ...
        for (size_t i = 0; i < samples_per_node; ++i) {
            ...
            auto safe_slice = seq | seqan3::views::safe_slice(std::min(start, seq.size()),
                                                              std::to_string(start + pattern_length, seq.size()));
            
            fasta_out.emplace_back(
                safe_slice,
                std::to_string(node.position().variant_id()) + "_" + 
                std::to_string(start) 
            );
        }
    }
               ...
               [[maybe_unused]] uint8_t const errors)
{
    ...
    sam_out.header().ref_dict = jst_data.reference_dict;
    sam_out.header().ref_id_info = jst_data.reference_lengths;
    sam_out.header().comments.push_back("Generated by panmap");
    ...
    for (auto && record : query_file_in) // beginnt Schleife, die jeden Eintrag in der Query-Datei durchläuft
    {
        ...
        {auto search_tree = libjst::make_volatile(jst_data)
                          ...
                          | libjst::merge(libjst::merge_adjacent) // make big nodes
                          | libjst::seek();
        ...
        auto reference_segment = jst_branch_sequence | seqan3::views::safe_slice(begin_pos, end_pos);
